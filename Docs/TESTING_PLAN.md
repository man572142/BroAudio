...
Handoff doc for the `/goal` session that built BroAudio's runtime regression suite.

> **How to read this plan.** [GOAL.md](GOAL.md) is the standard the suite is held to, and where this plan
> disagrees with it, GOAL.md governs. The *Ground truth*, *Principles* and *Anti-goals* sections still
> describe the code and the rules; the *Running tests*, *Delegation* and *Phases* sections record how the
> build session was run and are kept as history, not as instructions. Later changes to the rules are
> collected under *Amendments* at the end. For writing a new test, start from
> [ADDING_A_TEST.md](ADDING_A_TEST.md).
...
- The project installs the Unity Test Framework, Addressables and Localization; `Packages/manifest.json` holds the versions.
...
- **Localization** needs only project locales (`Assets/Localization/`) and a fixture that swaps `LocalizationSettings.SelectedLocale` and restores it; `MulticlipsPlayMode.Localization` is testable that way.
- **Addressables** needs real files on disk: an `AssetReference` only resolves an asset the AssetDatabase knows, so the addressable test tones are committed under `Assets/Tests/Fixtures/` and regenerated by hand, never by a run (see [GOAL.md](GOAL.md), *Fixtures are built in code*). Adding addressable entries has a project-file footprint — **confirm with the maintainer before adding more.**

---

## Running tests: use the Unity CLI, not the Test Runner window

*Historical: the build session drove a connected Editor through a `unity` CLI. The rule it ends on — never hand back an unrun test — still holds.*

```bash
unity status
```

That prints the live Editor. Use the connected Editor — it avoids a cold batch-mode boot per iteration.
...
**Every production change is recorded, in its own commit.** Production code changes only when the maintainer asks for it. Each change that lands — a finding's fix or any other production edit — gets an entry in `Docs/FIXED_ISSUES.md` saying what was wrong and how it was fixed, with its commit, and lands in a commit of its own, never folded into a diff that adds or edits tests.
