...
- `SeamlessLoopHelper.cs` (`Assets/BroAudio/Runtime/SoundManager/SeamlessLoopHelper.cs`) is entirely commented out — dead file, not wired into the seamless-loop mechanism described above (which lives in `AudioPlayer.Playback.cs`/`AudioPlayer.Scheduling.cs` instead). Not a behavior to test; noted so nobody goes looking for logic there.
